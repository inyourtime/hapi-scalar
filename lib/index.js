...
/**
 * @typedef {import('@scalar/client-side-rendering').HtmlRenderingConfiguration} HtmlRenderingConfiguration
 */
...
const hapiScalar = async (server, options) => {
  ...
  server.route({
    ...
    handler: async (request, h) => {
      ...
      const { pageTitle, cdn, ...config } = {
        ...defaultScalarConfig,
        ...scalarConfig,
      }

      const html = renderApiReference({ config, pageTitle, cdn })
